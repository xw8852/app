...
public class RiseBerzierCurvesAnimation extends BaseBerzierCurvesAnim {
    ...
    protected void transformation(float interpolatedTime, Transformation t) {
        interpolatedTime = Math.round(interpolatedTime * 100) / 100.0f;
        int count = points.length;
        PointF[] _origin = new PointF[count];
        for (int i = 0; i < count; i++) {
            if (type == TYPE_X) {
                int _x = (int) ((points[i].x - origin) * interpolatedTime);
                _origin[i] = new PointF((int) (_x + origin), points[i].y);
            } else {
                int _y = (int) ((points[i].y - origin) * interpolatedTime);
                _origin[i] = new PointF(points[i].x, (int) (_y + origin));
            }
        }
